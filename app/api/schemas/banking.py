...
class BankingInstrument(BaseModel):
    ...
    interest_rate: float | None = Field(
        default=None, ge=0, description="Процентная ставка (для вкладов и накоплений)"
    )
    opened_at: datetime = Field(description="Дата открытия")
    expires_at: datetime | None = Field(
        default=None, description="Дата окончания (для вкладов)"
    )
    days_remaining: int | None = Field(
        default=None, ge=0, description="Дней до окончания (для вкладов)"
    )
...
class BankingInstrumentsResponse(BaseModel):
    ...
    total_balance: int = Field(
        ge=0, description="Общий баланс всех инструментов в копейках"
    )
