...
class DepositDetail(BaseModel):
    ...
    days_remaining: int = Field(ge=0, description="Остался срок вклада в днях")
    interest_payment_method: InterestPaymentMethod = Field(description="Способ выплаты процентов")
...
class DepositCreateRequest(BaseModel):
    ...
    term_days: int = Field(ge=1, le=1095, description="Срок вклада в днях (максимум 3 года)")
    interest_rate: float = Field(gt=0, le=25.0, description="Процентная ставка (не более 25%)")
    interest_payment_method: InterestPaymentMethod = Field(description="Способ выплаты процентов")
    ...
    def validate_interest_rate(cls, v: float) -> float:
        ...
        if v > key_rate:
            raise ValueError(f"Процентная ставка не может превышать ключевую ставку {key_rate}%")
        return v
...
class DepositCloseResponse(BaseModel):
    ...
    penalty_applied: bool = Field(description="Была ли применена пеня за досрочное закрытие")
...
class DepositTransaction(BaseModel):
    ...
    id: uuid.UUID
    name: str = Field(description="Название операции (открытие, проценты, досрочное закрытие)")
